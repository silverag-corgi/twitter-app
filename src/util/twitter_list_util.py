    ...
    try:
        ...
        for twitter_list in twitter_lists:
            if twitter_list.name == list_name:
                logger.info(f'Twitterリストが既に存在します。(リスト名：{twitter_list.name})')
                return True
...
def generate_twitter_list(api: tweepy.API, twitter_list_name: str) -> tweepy.List:
    ...
    try:
        ...
        logger.info(f'Twitterリスト作成に成功しました。(リスト名：{twitter_list.name})')
    except Exception as e:
        logger.info(f'Twitterリスト作成に失敗しました。')
        raise(e)
...
def add_user(api: tweepy.API, twitter_list: tweepy.List, user_id: str, user_name: str) -> None:
    ...
    try:
        ...
        logger.info(f'ユーザ追加に成功しました。(ユーザID：{user_id: <20}、ユーザ名：{user_name})')
    except Exception as e:
        # ユーザが鍵付きや削除済みなどの場合
        logger.info(f'ユーザ追加に失敗しました。(ユーザID：{user_id: <20}、ユーザ名：{user_name})')
