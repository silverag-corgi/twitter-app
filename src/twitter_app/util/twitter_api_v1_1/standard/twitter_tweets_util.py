...
def search_tweets_in_past_7day(
    ...
    num_of_data_per_request: int = EnumOfTweetsInPast7Day.EnumOfOauth1User.MAX_NUM_OF_DATA_PER_REQUEST.value,
) -> list[SearchResults]:
    """
    ツイート検索(過去7日間)
...
class CustomStream(tweepy.Stream):
    ...
    def on_status(
        ...
        try:
            ...
            if hasattr(tweet, "retweeted_status") is True:  # リツイートの場合
                ...
                if self.__following_user_ids is None:
                    ...
                    tweet.in_reply_to_user_id not in self.__following_user_ids
